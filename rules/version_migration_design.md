...
DocAdvisor の設定ファイル（YAML）マイグレーションについては:
`specs/design/DES-002_config_migration.md` を参照。

現行の v4.x 実装（`merge_config.py`）はパイプライン原則を**完全には満たしていない**。
v5 移行時に同ファイルの改修が必要。詳細は DES-002 に記載。
