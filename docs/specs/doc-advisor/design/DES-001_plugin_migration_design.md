...
> **歴史的記録（移行完了済み・スナップショット）**: 本文書は DocAdvisor を初期の setup.sh 配布方式から `bw-cc-plugins` マーケットプレイスへ第一次移行した際の設計記録である。**現在は更に進んで単一プラグインリポジトリ（本リポジトリ）に分離済み**。文中の「plugins/doc-advisor/ 配下」のディレクトリ構造はマーケットプレイス時代の前提であり、現状はルート直下が `${CLAUDE_PLUGIN_ROOT}` となる。歴史的経緯の参照用として保持しているが、現状の構造は `README.md` と `CLAUDE.md` を参照すること。
