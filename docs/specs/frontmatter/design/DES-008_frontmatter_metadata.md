...
- `fm_to_pending.py` は pending の**形式は知るが、置き場所は知らない**。処理対象はコマンド引数で受け取る
- 呼び出し側（`index-docs` SKILL / `prepare_toc.py`）が場所を決めて渡す

`fm_to_pending.py` は 2 つの処理単位を持つ。

| オプション         | 処理単位     | 挙動                                                                                |
| ------------------ | ------------ | ----------------------------------------------------------------------------------- |
| `--out <path>`     | 1 ファイル   | 判定対象 1 件の結果を指定先へ pending として書き出す                                |
| `--work-dir <dir>` | ディレクトリ | 指定ディレクトリ直下の pending を一括処理し、信頼できるものを in-place で完了化する |

`--work-dir` は、そのディレクトリ直下にある pending を列挙し、各 pending の `_meta.source_file` が指す文書を §5.1 の述語で判定する。信頼できるものはその pending をその場で `status: completed` に更新し、信頼できないものは変更せず AI 抽出の対象として残す。

`--work-dir` を持たせる理由は 2 つある。ディレクトリ内の pending の列挙は決定論的な定型処理であり、AI に列挙・手転記をさせないため script 側に置く。かつ、渡されたディレクトリを走査するだけで key 解決も store_dir 解決も行わないため、本節の独立性の境界を保ったまま 1 コマンドで済む。
...
| `fm_to_pending.py` | 信頼できるフロントマターを pending YAML（`status: completed`）にする。`--out` は 1 件、`--work-dir` は一括 |
...
適用対象は `docs/` 配下の文書とし、`plugins/doc-advisor/` 配下の配布物は**対象に含めない**。配布物の SKILL.md のフロントマターは Claude Code 仕様に従うため、独自キーの追加が許容されるかが仕様側に依存し、doc-advisor の側では担保できないためである。

この除外は対象を列挙する側（`fm_read.py` の走査モード）に持たせる。`fm_write.py` は対象を選ばない汎用ツールとして保ち、方針の変更が 1 箇所で済むようにする。
