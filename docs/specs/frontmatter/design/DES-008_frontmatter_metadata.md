...
keywords:
  ...
  - extracted_by
  - format-command
body_hash: sha256:67a0e32d425bd88d115b51ac7c7db96a5edd0010b6aef3cf8cdaf8e0ab3f6a8f
---
...
purpose: What this document exists for, aiming for 200 characters (hard limit 220)
content_details:
...
変換は**値が入ってくる入口**で行う。入口は 2 つある。

| 入口                              | 何が入ってくるか                    | 報告先                                       |
| --------------------------------- | ----------------------------------- | -------------------------------------------- |
| `fm_write.write_entry`（手順 0a） | 原本フロントマターへ書くメタデータ  | `normalized_fields` → `fm_run` の `warnings` |
| `write_pending.write_entry_yaml`  | AI が書いた値（ToC パイプラインへ） | stderr（`toc-updater` の実行ログ）           |

**変換したことを必ず報告する。** 書いた値と原本・`toc.yaml` に入る値が違うことを黙って済ませない。変換規則そのものは中心側（`toc_utils.normalize_field_value`）に 1 つ置き、派生側は「どのキーが 5 フィールドか」だけを足す（規則を 2 実装に分けると片方だけが改訂される）。

**値域の検証（`\` の拒否）が効くのは、フロントマターへの書き込みと ToC からの転記の読み取りである。** `toc.yaml` への書き込み点（`write_pending` / `merge_toc`）は変換のみを通し、検証は行わない。したがって `\` を含む値が `toc.yaml` へ入ることは形式上ありえ、その場合そのエントリは転記時に恒久的に `incomplete_entry` となる。**この経路に検証点を足していないのは、AI が英語のメタデータへ `\` を書く事象が観測されていないためである**（既存 ToC 1392 値の走査で該当 0 件）。起きていない事象に対して 2 つ目の検証点を置くと、規則の実装が 2 箇所に増えるという既知の害の方が確実に生じる。観測されたら足す。
...
`purpose` の 200 文字という目標値は英語前提の値として維持する。ただし機械検査の受け入れ上限は 220 文字（+10%）とする。目標値と受け入れ上限を分ける理由は §5.1 の値域表に記す。
...
| `title`            | string                      | 非空                                                    |
| `purpose`          | string                      | 非空、220 文字以内（起草目標は 200 文字）               |
...
**`purpose` の起草目標（200 文字）と受け入れ上限（220 文字）を分ける理由**: 起草するのは LLM であり、LLM は文字数を正確に数えられない。目標値と受け入れ上限が同一だと、目標どおりに書こうとした僅差の超過が値域違反として弾かれる（実運用で 206 文字の超過を観測。ToC の書き込み点には検証が無いため `toc.yaml` へ入り、転記段階で恒久的に `incomplete_entry` となった）。+10% のマージンはこの計数誤差を吸収するためのものであり、起草の目標値を 220 文字へ引き上げるものではない（`toc_format.md` の Field Guidelines は目標 200 文字のまま）。定数は `fm_core.PURPOSE_GUIDELINE_LENGTH`（200）と `fm_core.PURPOSE_MAX_LENGTH`（220）。
...
- **値域検証の一方向包含（§6.2）**: 書き込み側が受理した metadata を書き込んだ結果に、読み取り側の**値域**判定が違反を返さないこと。あわせて値域違反の各種（文字数超過・件数超過・空・型不一致・配列内の空要素）で **対象ファイルのバイト列が 1 バイトも変わらない**ことを固定する。`changed` の値は script の自己申告であり、書かれていないことの証明にならないため、実ファイルを読み比べる。上限ちょうど（`purpose` 220 文字・配列 10 件）が違反にならない境界、および起草目標を僅かに超えた値（200+6 文字）が受理されることも固定する
...
そこで **AI が呼ぶ入口を 2 つのサブコマンドに畳む**。AI に残す責務は「メタデータの内容を作ること」と「`write_policy: confirm` の対象について書き込みの承認を取ること」だけである。承認の要否そのものも `plan` が `targets[].write_policy` で決定論的に分類する（§8.2 の承認方針）。
...
| `--exclude <path>...`          | 利用者                | 確定した対象集合からの除外（出どころを問わない。DES-005 §4.2.2）                  |
...
- ToC の生成完了後に、対象を提示して `write-frontmatter` へ引き渡す。承認の要否は文書ごとに `write-frontmatter` 側で判定する（下記「承認方針」）。`index-docs` が独自の確認ゲートを持つと二重承認になるため持たない
- 引き渡しのための候補は、pending の `_meta.extracted_by`（`frontmatter` | `ai`）に記録し、`merge_toc.py` の JSON 出力を通じて SKILL が参照する
- `extracted_by` は `toc.yaml` には書き出さない（`type` と同じ理由で、検索の識別情報ではないため）

なお「信頼できるフロントマターを持たないファイルの集合」は `fm_read.py` でコーパスを走査すればいつでも再計算できるため、この来歴は永続状態として保持する必要はない。実行中の引き渡しを簡便にするための一時情報である。

#### 承認方針（`write_policy`）

書き込み前のユーザー承認の要否は、`fm_run.py plan` が対象ごとに `targets[].write_policy` で決定論的に分類する。AI が判断しない。

| 値        | 条件                                                                         | 承認                             |
| --------- | ---------------------------------------------------------------------------- | -------------------------------- |
| `auto`    | 既存フロントマターが doc-advisor の標識（`type` に `doc-advisor`）を持つ     | 不要。対象と内容を提示のみで書く |
| `confirm` | フロントマターが無い、または他ツールのフロントマターしか持たない（標識なし） | 必須。`AskUserQuestion` で得る   |

`auto` を承認不要とする根拠: doc-advisor が過去に書いたフロントマターの値を最新の索引結果で刷新する操作であり、利用者にとって新しい種類の変更ではない。マージは doc-advisor が所有する 6 キーだけを差し替え、他ツールのキーは原文行のままバイト保持されるため（§4.5）、自動書き込みが他ツールの情報を壊すことはない。一方 `confirm` は「その文書にフロントマターという構造を新たに導入する」または「他ツールの領域に doc-advisor のキーを初めて追記する」操作であり、文書の見た目が変わる判断はユーザーに残す。

counts には `auto` / `confirm` の件数を常に出す（`targets` を write_policy で分割した値）。
...
| 2026-08-29 | 2.3        | **`purpose` の起草目標（200 文字）と受け入れ上限（220 文字）を分離した**（§4.1 / §4.4 / §5.1 / §6.4）。LLM は文字数を正確に数えられず、目標どおりに書こうとした僅差の超過（実運用で 206 文字を観測）が値域違反となり、ToC の書き込み点には検証が無いため当該エントリが転記段階で恒久的に `incomplete_entry` になっていた。受け入れ判定に +10% のマージンを取り計数誤差を吸収する。起草目標は 200 文字のまま（`toc_format.md` Field Guidelines）。定数は `PURPOSE_GUIDELINE_LENGTH` / `PURPOSE_MAX_LENGTH`                                                                                                                                                                                                                                  |
| 2026-08-29 | 2.2        | **書き戻しの承認方針（`write_policy`）を §8.2 に規定した**。doc-advisor の標識を持つ既存フロントマターの更新は承認不要（`auto`）、フロントマターの新規追加・他ツールのみの文書への追記は承認必須（`confirm`）とし、分類は `fm_run.py plan` が `targets[].write_policy` と `counts.auto` / `counts.confirm` で決定論的に返す。従来は既存の有無によらず常に承認を取り、さらに `index-docs` が独自の確認ゲートを持つ二重承認だった。`index-docs` のゲートを撤去し、承認の判定と取得を `write-frontmatter` 側へ一元化した（§6.5 / §8.2）                                                                                                                                                                                                       |
