...
| 値域外の文字     | 扱い                   | 理由                                                                                                                  |
| ---------------- | ---------------------- | --------------------------------------------------------------------------------------------------------------------- |
| `"`              | **変換**（`` ` `` へ） | 英語の記述文で意味は変わらない                                                                                        |
| 改行 / CR / タブ | **変換**（空白へ）     | フィールドは単一行と定義済みであり、空白へ畳んでも意味は変わらない                                                    |
| 先頭・末尾の `'` | **変換**（`` ` `` へ） | 読み側が両端の引用符を削るためこの位置だけが問題になる。`don't` のような中間の `'` は温存する                         |
| `\`              | **拒否**（代替が無い） | 落とせば `\n handling` が `n handling` に、`/` へ替えればエスケープ表記が path 表記に変わる。値からどちらか決まらない |

変換は書き込みの入口 1 箇所（`fm_write` の手順 0a）で行い、**変換したことを必ず報告する**（`normalized_fields` → `fm_run` の `warnings`）。書いた値と原本に入る値が違うことを黙って済ませない。変換規則そのものは中心側（`toc_utils.normalize_field_value`）に 1 つ置き、派生側は「どのキーが 5 フィールドか」だけを足す（規則を 2 実装に分けると片方だけが改訂される）。

**読み取り側は値域の全体を引き続き検査する。** 人間が原本のフロントマターを手で編集して `"` を入れた場合は「信頼できない」と判定して再抽出へ回す必要があり、あわせて「書ける集合 ⊆ 信頼される集合」（§6.2）の事後条件としても機能する。書き込み側の変換はこの包含を**強める**方向に働く。

内容を選べない値（pending の `_meta.error_message` に入る捕捉済み例外文字列）には値域を課せないため、**入口で正規化する**（引用符はバッククォートへ、改行・タブは空白へ、バックスラッシュはここでは削除する）。診断は非可逆な正規化を許容できるが、データは許容できない。この非対称が両者を分けて扱う理由である。
...
**現時点で上位層（forge / anvil）からの呼び出し元は存在しない。** 唯一の呼び出し元は `index-docs` の書き戻し経路であり、`--paths`（承認された対象）と `--from-toc <key>`（今回索引した key）の 2 つを使う。上位層への契約反映は §10.3 のとおり未決であり、**契約が生じた時点で本表と DES-005 §10.1 の「既知の呼び出し元」を同時に更新する**。
