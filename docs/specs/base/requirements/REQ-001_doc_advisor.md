...
| FR-N04-5 | 索引入口では、単体モード（`--all` / `--key` 省略）と対象指定（`--dirs` / `--paths` およびその JSON 形 / `--paths-file`）・`--exclude` の併用を **reject する**（`UNSUPPORTED_ARG`）。黙って一方を捨てない |
...
>
> **単体モードへの入口は 2 つあり、両者は同義である（FR-N04-5 の根拠）**: `--all` の明示と `--key` の省略は、key 解決だけでなく**走査の起動においても**同義である。したがって単体モードに課される制約（対象指定・`--exclude` との併用 reject）は、**どちらの入口から入った場合にも等しく適用する**。実装で `--all` フラグの有無だけを見ると `--key` を省略した経路が制約を素通りし、渡された対象指定が走査分岐で捨てられる。索引は desired-state であるため、**「1 件だけ索引する」つもりの指定が当該 key の ToC を全件へ置き換える**結果になる。
>
> 検索・取得の入口（`get_toc` / `query-docs`）はこの制約を受けない。走査を行わないため対象指定を捨てる余地が無く、`--key` 省略時も `--paths` の縮小抽出は尊重される。制約が生じるのは**走査を伴う索引入口だけ**である。
...
| FR-N09-8 | `--dirs-json` のエントリがグロブメタ文字（`*` `?` `[`）を含む場合、グロブパターンとして展開する。マッチしたディレクトリは rglob で配下 Markdown を収集し、マッチした Markdown ファイルは直接採用する。`..` を含むパターン・絶対パスのパターンは reject（`rejected_dirs`）する。グロブメタ文字を含まないエントリは従来どおりリテラルディレクトリとして扱う（後方互換） |
...
| **symlink** | **渡されたものは索引する。** 実体を解決し、解決後パスが project root 配下なら受理。root 外を指す symlink も、**対象として渡された**ものであれば受理し、解決先と件数を warning に出す。確認するのは単体モード（`--all` / `--key` 省略）の走査で見つかったものだけである（§6.1a / 実装は DES-005 §5） |
...
> **前提**: 「project root の外に実体を置き symlink で取り込む」運用（例: 別リポジトリの共有仕様書、共通ルール集）は**実運用で使われている**。その実体が root 外にある以上、symlink 以外に索引する経路がない（絶対パスは `ABSOLUTE_PATH`、root 外への相対は `PATH_TRAVERSAL` で reject されるため）。実体解決には `Path.is_relative_to`（3.9 で追加）を使う。サポート下限そのものは NFR-N01 のとおり保守期限で決める。実体解決の実装詳細は DES-005 §5 に委ねる。

### 6.1a 越境 symlink の決定権は呼び出し元にある【確定】

**渡された対象は、越境 symlink であっても索引する。** doc-advisor は索引の実行主体であり、何を索引するかを決める主体ではない。

判定基準は **単体モードか否か** の 1 点である。対象指定の形（`--dirs` / `--dirs-json` / `--paths` / `--paths-json` / `--paths-file`）による違いはない。

| 経路                                                                                                         | 扱い                                        | 根拠                                                                                      |
| ------------------------------------------------------------------------------------------------------------ | ------------------------------------------- | ----------------------------------------------------------------------------------------- |
| **`--all` 以外のすべての対象指定**（`--dirs` / `--dirs-json` / `--paths` / `--paths-json` / `--paths-file`） | **索引する**。解決先と件数を warning に出す | 呼び出し元が索引対象として渡したものであり、それが symlink であることは渡す側が知っている |
| `--all`（`--key` 省略も同義。project root 全体の走査）                                                       | ユーザーに確認する（`needs_confirmation`）  | 誰も対象として渡していない。project root の外へ勝手に広げない                             |

**渡された対象について確認を挟んではならない理由**は 2 つある。
...
| グロブ展開          | `--dirs-json` エントリがグロブメタ文字（`*` `?` `[`）を含む場合はパターン展開（`pathlib.Path.glob`）。マッチしたディレクトリは rglob、Markdown ファイルは直接採用。`..`・絶対パスは reject（FR-N09-8） |
...
| ID      | 要件                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| ------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| NFR-N01 | Python は標準ライブラリのみ使用（NFR-003 を継続）。**サポート下限は Python 3.11**。下限は文法上の必要（`Path.is_relative_to` は 3.9 で追加）ではなく **upstream のセキュリティ保守期限**で決める。3.9 は 2025-10-31、3.10 は 2026-10-31 に EOL であり、EOL 版を下限に掲げることは未修正 CVE 上での実行を公式に許可することを意味する。macOS 標準の `/usr/bin/python3`（3.9）はサポート対象外とし、README / guide に明記する。下限と最新版の双方で CI がテストを実行する |
...
| NFR-N06 | （セキュリティ）path traversal によるインデックス漏洩を防止する。検証方針は §6.1 path validation policy に従う（traversal は論理パス検証で reject）。**root 外を指す symlink は、対象として渡されたものであれば索引し、warning で注意喚起する**（索引するか否かの決定は呼び出し元に属する。§6.1a）。単体モード（`--all` / `--key` 省略）でのみ、誰も渡していない越境 symlink についてユーザーに確認する                                                                 |
...
- [ ] **`--all` 以外のすべての対象指定（`--dirs` / `--dirs-json` / `--paths` / `--paths-json` / `--paths-file`）は、root 外を指す symlink 経由であっても索引され**、解決先の実体パスと件数を含む warning が出る（NFR-N06 / §6.1a）
