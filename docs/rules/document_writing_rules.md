...
1. `docs/rules/` / `docs/specs/`（本文） / `workflows/` / `formats/` / `skills/*/SKILL.md` 等は **現在状態**のみを記述する。移行・廃止のナラティブを本文に残さない
2. 移行・廃止の履歴は `CHANGELOG.md` に置く。各 spec 末尾の「変更履歴」表は、その文書自身の履歴として保持してよい
3. 旧名・旧構造への参照を見つけたら、現在主語へ書き換えるか削除する

**例外（旧名を本文に残してよい場合）:**

- 旧名が要件の機能的主語であるもの（例: レガシー検出要件は検出対象として旧名が必要）
- ADR 等で過去事例が決定の根拠であるもの。ただし決定は現在主語に直し、事例は史実を歪めない範囲で汎用化する

**DO NOT**: 移行完了後に「旧 X → 新 Y」の対応表や経緯説明を本文へ残置しない。spec を削除・縮約する前に、関連する方針文書（例: `docs/rules/version_migration_design.md`）を読む。
