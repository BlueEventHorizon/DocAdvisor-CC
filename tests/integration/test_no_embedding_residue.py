...
# 配布物（scripts / skills / workflows / formats / .claude-plugin）は plugins/doc-advisor/ 配下に置く。
...
class TestNoEmbeddingResidue(unittest.TestCase):
    ...
    def test_code_index_dir_absent(self):
        ...
        self.assertFalse(
            (PLUGIN_ROOT / "scripts" / "code_index").exists(),
    ...
    def test_index_workflow_absent(self):
        ...
        self.assertFalse(
            (PLUGIN_ROOT / "workflows" / "query_index_workflow.md").exists(),
            "workflows/query_index_workflow.md が再混入している",
        )

    def test_toc_utils_has_no_index_file(self):
        """toc_utils.py に index_file 設定面が残っていない。"""
        content = (PLUGIN_ROOT / "scripts" / "toc_utils.py").read_text(encoding="utf-8")
        self.assertNotIn(
            "index_file",
            content,
    ...
    def test_runtime_docs_have_no_embedding_script_refs(self):
        ...
        for subdir in ("workflows", "formats"):
            for path in (PLUGIN_ROOT / subdir).glob("*.md"):
