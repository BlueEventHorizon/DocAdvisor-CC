---
description: Generate AI-searchable structured ToC from requirements and design documents (documents)
---
...
2. doc_type determination from path (`reqs/` → `requirement`, `arch/` → `design`)
3. Filename conversion (e.g., `documents/main/reqs/login.md` → `documents_main_reqs_login.yaml`)
...
```bash
# Target file list (reqs/ and arch/ subdirectories only)
find documents \( -path "*/reqs/*.md" -o -path "*/arch/*.md" \) | grep -v ".toc_work" | grep -v "reference" | grep -v "/info/" | sort

# Calculate hash for each file
shasum -a 256 documents/main/reqs/app_overview.md | cut -d' ' -f1
```
...
Task(subagent_type: specs-toc-updater, prompt: "entry_file: .claude/doc-advisor/specs/.toc_work/documents_main_reqs_login.yaml")
Task(subagent_type: specs-toc-updater, prompt: "entry_file: .claude/doc-advisor/specs/.toc_work/documents_main_reqs_user_profile.yaml")
Task(subagent_type: specs-toc-updater, prompt: "entry_file: .claude/doc-advisor/specs/.toc_work/documents_main_arch_login_screen.yaml")
Task(subagent_type: specs-toc-updater, prompt: "entry_file: .claude/doc-advisor/specs/.toc_work/documents_main_arch_api_design.yaml")
Task(subagent_type: specs-toc-updater, prompt: "entry_file: .claude/doc-advisor/specs/.toc_work/documents_auth_reqs_oauth.yaml")
```
...
_meta:
  status: error
  source_file: documents/main/reqs/screens/login_screen.md
