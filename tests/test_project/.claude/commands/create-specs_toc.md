---
description: Generate AI-searchable structured ToC from requirements and design documents (specs)
---
...
2. doc_type determination from path (`requirements/` → `requirement`, `design/` → `design`)
3. Filename conversion (e.g., `specs/main/requirements/login.md` → `specs_main_requirements_login.yaml`)
...
find specs \( -path "*/requirements/*.md" -o -path "*/design/*.md" \) | grep -v ".toc_work" | grep -v "reference" | grep -v "/info/" | sort

# Calculate hash for each file
shasum -a 256 specs/main/requirements/app_overview.md | cut -d' ' -f1
```
...
Task(subagent_type: specs-toc-updater, prompt: "entry_file: .claude/doc-advisor/specs/.toc_work/specs_main_requirements_login.yaml")
Task(subagent_type: specs-toc-updater, prompt: "entry_file: .claude/doc-advisor/specs/.toc_work/specs_main_requirements_user_profile.yaml")
Task(subagent_type: specs-toc-updater, prompt: "entry_file: .claude/doc-advisor/specs/.toc_work/specs_main_design_login_screen.yaml")
Task(subagent_type: specs-toc-updater, prompt: "entry_file: .claude/doc-advisor/specs/.toc_work/specs_main_design_api_design.yaml")
Task(subagent_type: specs-toc-updater, prompt: "entry_file: .claude/doc-advisor/specs/.toc_work/specs_auth_requirements_oauth.yaml")
```
...
_meta:
  status: error
  source_file: specs/main/requirements/screens/login_screen.md
