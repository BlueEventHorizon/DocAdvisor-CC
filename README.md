...
| **write-frontmatter** | 文書へ検索用メタデータをフロントマターとして書き込む（原本を変更） | `"フロントマターを書き込む"` |
...
### 4. メタデータの埋め込み（write-frontmatter）

索引は既定では文書ごとに本文を読んでメタデータを抽出する（コールドリード）。**文書自身にメタデータをフロントマターとして持たせておくと、`index-docs` はそれを転記するだけで済み、Agent の起動を省略できる。** 大量文書の初回索引が重い場合に効く。

```text
# 対象を指定して書き込む（原本の Markdown が変更される）
/doc-advisor:write-frontmatter --dirs docs/rules/

# 整形器があれば渡す（整形の後に body_hash を打刻するため）
/doc-advisor:write-frontmatter --paths docs/a.md --format-command "dprint fmt {file}"
```

- **原本を書き換えるスキル**である。doc-advisor のフロントマターを既に持つ文書の更新は、対象と内容を提示のうえ承認なしで書き込む。フロントマターが無い文書・他ツールのフロントマターしか持たない文書への新規追加は、対象一覧とメタデータを提示して承認を得てから書き込む
- 既存のフロントマターのキーは保持し、`type` は他ツールの標識を消さずに `doc-advisor` を追加する
- 本文が変わると `body_hash` が一致しなくなり、そのメタデータは信頼されない（自動的に AI 抽出へ戻るため、古いメタデータで索引されることはない）
- `index-docs` は ToC の生成完了後に、AI 抽出で索引された文書を書き戻し候補として本スキルへ引き渡す（承認の要否は本スキルが文書ごとに判定する）
