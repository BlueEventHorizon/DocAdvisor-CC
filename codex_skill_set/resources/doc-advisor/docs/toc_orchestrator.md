...
The toc-updater reference subagent writes error information to the YAML file (status remains `pending`) before returning `❌ Error`.
...
```yaml
# Example of error YAML (written by toc-updater reference via write_pending.py --error)
_meta:
  status: pending
