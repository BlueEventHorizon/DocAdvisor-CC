...
**追加開発（`--add`）の場合は以下も [MANDATORY] Read**:

- `$DOC_ADVISOR_CODEX_ROOT/resources/forge/docs/additive_development_spec.md` — 適用条件・優先度・新規ファイル分離・merge 手順
- `$DOC_ADVISOR_CODEX_ROOT/resources/forge/docs/requirement_format.md` — `type: temporary-feature-requirement` frontmatter 定義
...
PYTHONDONTWRITEBYTECODE=1 python3 "$SCAN_SCRIPT" SCR   # 画面要件の場合
PYTHONDONTWRITEBYTECODE=1 python3 "$SCAN_SCRIPT" FNC   # 機能要件の場合
# 使用するプレフィックスごとに実行する
```
...
`create-specs-toc` が利用可能であれば実行する（利用不可の場合はスキップ）。
...
| 問題タイプ | 条件                                                       | 重要度                                           |
| ---------- | ---------------------------------------------------------- | ------------------------------------------------ |
| 曖昧表現   | TBD-xxx として完全明示済み（ID・内容・期限すべて記載）     | 検出対象外（誤検出時のみ skip / false_positive） |
| 曖昧表現   | 未確定事項表に部分明示（散文の委譲記述のみ・期限欠落など） | 🟡 品質                                          |
| 曖昧表現   | 未確定事項に未登録                                         | 🟡 品質                                          |
| 曖昧表現   | ユーザー視点の要件（What レイヤ）が曖昧                    | 🔴 致命的                                        |
| How 混入   | 全て                                                       | 🟡 品質                                          |
