...
```bash
python3 .claude/doc-advisor/scripts/embed_docs.py --category {category}
```
...
1. セマンティック検索を実行する:
   ```bash
   python3 .claude/doc-advisor/scripts/search_docs.py --category {category} --skip-stale-check --query "{task}"
   ```
2. タスク説明に固有名詞・識別子が含まれる場合は全文検索を補足する:
   ```bash
   python3 .claude/doc-advisor/scripts/grep_docs.py --category {category} --keyword "{固有名詞}"
   ```
...
- **"Model mismatch"** → `--full` で再構築を試行:
  ```bash
  python3 .claude/doc-advisor/scripts/embed_docs.py --category {category} --full
  ```
