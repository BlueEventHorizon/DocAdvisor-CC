...
description: |
  ...
  信頼判定は fm_run.py が行う。doc-advisor のフロントマターを既に持つ文書
  （write_policy: auto）は提示のみで自動的に書き戻し、フロントマターが無い
  文書・他ツールのフロントマターしか持たない文書（write_policy: confirm）は
  対象一覧とメタデータを提示して承認を得てから書き込む。
  トリガー:
...
1. **書き込みの承認を取ること** — **`write_policy: confirm` の対象だけ**（フロントマターが無い文書への新規追加、または他ツールのフロントマターしか持たない文書への追記）。`write_policy: auto` の対象（doc-advisor の標識を持つ既存フロントマターの更新）は承認不要で、提示のみで書き込む
2. **メタデータの内容を作ること** — **ToC から転記できなかった対象だけ**、本文を読んで 5 フィールドを書く

承認の要否は AI が判断しない。`plan` が `targets[].write_policy` で決定論的に分類する。
...
| 何が起きるか                                                        | 発生条件                                                                                                                                                 |
| ------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 原本のフロントマターへ doc-advisor のキーがマージ書き込みされる     | `write_policy: auto` の対象は Step 3 で提示した後（承認不要）。`write_policy: confirm` の対象は Step 3 の `AskUserQuestion` でユーザーが承認したときのみ |
| `--format-command` が対象ファイルに対して実行され、本文が整形される | Step 3 を経た後、かつ `--format-command` が指定されているときのみ                                                                                        |
| `body_hash` が打刻される（整形の後）                                | Step 3 を経た後、書き込みが成功したときのみ                                                                                                              |
| 対象以外のファイルの変更                                            | **起きない**（script は渡されたパスしか触らない）                                                                                                        |

- **`write_policy: confirm` の対象を含むまま、承認を得る前に `fm_run.py apply` を実行してはならない**。`plan` と Step 2 は読み取りのみで、原本は 1 バイトも変わらない
- `write_policy: auto` の対象は doc-advisor 自身が過去に書いたフロントマターの更新であり、承認なしで書き戻す。他ツールのキーは値を変更せず保持されるため、自動書き込みが壊すものはない
...
- `${CLAUDE_PLUGIN_ROOT}/formats/toc_format.md` — メタデータ 5 フィールドの内容規約（Field Guidelines。`purpose` は目標 200 文字・受け入れ上限 220 文字・各配列 10 件・`keywords` の書き方）と、フィールド値の言語規定（Language Rule）
...
| `targets[].write_policy`   | `auto` = doc-advisor の標識を持つ既存フロントマターの更新（**承認不要**）／ `confirm` = 新規追加・他ツールのみ（**承認必須**） |
...
| `counts`                   | `total` / `targets` / `from_toc` / `needs_ai` / `auto` / `confirm` / `skipped` / `unreadable`                                  |
...
| `purpose`          | 非空、目標 200 文字（受け入れ上限は 220 文字）。その文書が何のためにあるか     |
...
作成したメタデータは **`Write` ツールで JSON ファイルへ書き出す**。形式は次のとおりで、Step 4 の対象になるもの（`write_policy: auto` の対象と、Step 3 で承認された `confirm` の対象）のみを入力順に並べる。
...
**承認の要否は `targets[].write_policy` に従う。自分で判断しない。**

まず全対象を `write_policy` 別に提示する。各文書のメタデータ全文と `--format-command` の値を添え、メタデータは出どころを示す（`source: toc` は Step 1 の `targets[].metadata`、`source: ai` は Step 2 で作成したもの）。

- **`write_policy: auto` の対象**（doc-advisor の標識を持つ既存フロントマターの更新）: 提示のみで承認を取らず、そのまま Step 4 の対象に含める。原本に diff が生じることは提示の中で明示する
- **`write_policy: confirm` の対象**（フロントマターが無い文書への新規追加・他ツールのフロントマターしか持たない文書への追記）: `AskUserQuestion` で承認を得る

| 選択（confirm 対象への確認） | 動作                                                                                               |
| ---------------------------- | -------------------------------------------------------------------------------------------------- |
| **書き込む**                 | confirm 対象全件を auto 対象に加えて Step 4 へ進む                                                 |
| **対象を絞って書く**         | 除外する対象を `AskUserQuestion` で確認し、`--paths`（転記分）と JSON（起草分）から外して進む      |
| **書き込まない**             | confirm 対象は書き込まない。auto 対象のみで Step 4 へ進む（auto 対象も無ければ書き込まず終了する） |

- `counts.confirm` が 0（全件 auto）なら `AskUserQuestion` を出さず、提示のうえ Step 4 へ進む
- confirm 対象の承認が得られない場合、その対象を Step 4 に含めてはならない
...
**転記側の apply には、Step 3 を経た対象（`auto` 全件 + 承認された `confirm`）を `--paths` で必ず明示する [MANDATORY]**。`--from-toc` の apply は plan と同じ手順で対象を確定し直すため、`--paths` を省くと **ToC の全文書**を対象にする。Step 3 で対象を絞った場合・ユーザーが一部を除外した場合、省略すると承認されなかった対象まで原本が書き換わる。
...
- 承認区分: 自動書き戻し {Step 1 の counts.auto} / 承認対象 {Step 1 の counts.confirm}
...
- ❌ **`write_policy: confirm` の対象について Step 3 の承認を得ずに `apply` へ含めること**（`auto` の対象は提示のうえ承認なしで書き込む。この区分を AI が判断・変更してはならない）
