...
オプション:
    --file    検査対象ファイル（デフォルト: .claude/doc-advisor/toc/rules/rules_toc.yaml）
...
def init_config():
    ...
    # root_dirs_config が空の場合は PROJECT_ROOT / 'rules' をフォールバックとして使用する
    RULES_DIR = PROJECT_ROOT / root_dirs_config[0].rstrip('/') if root_dirs_config else PROJECT_ROOT / 'rules'
